(function(root){
  ...
  function awardBattle(pet,result){let p=gainExp(advance(pet),result.exp||0);const stat=result.won?'attack':'defense';p.evs[stat]=Math.min(252,(p.evs[stat]||0)+(result.won?2:1));p[result.won?'wins':'losses']=(p[result.won?'wins':'losses']||0)+1;p.stats.hunger=clamp(p.stats.hunger-6);p.stats.energy=clamp(p.stats.energy-10);p.stats.hygiene=clamp(p.stats.hygiene-4);p.stats.happiness=clamp(p.stats.happiness+(result.won?4:-5));if(!result.won)p.stats.health=clamp(p.stats.health-8);p.journal.unshift({at:Date.now(),text:`${result.won?'Won':'Completed'} a LAN battle and earned ${result.exp||0} EXP.`});if(result.item&&!p.items.includes(result.item))p.items.push(result.item);return p;}
